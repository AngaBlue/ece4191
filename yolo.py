...
def draw_detections(surface: pygame.Surface, det: dict | None, font: pygame.font.Font):
    ...
    # Limits bounding box to highest confidence only (unsureo of usefulness or if it works well)    
    # if not det or "boxes" not in det:
    #     return
    ...
    # max_idx = int(max(range(len(confs)), key=lambda i: confs[i]))

    # # Extract corresponding values
    # x1, y1, x2, y2 = boxes[max_idx]
    # cid = int(clss[max_idx]) if clss is not None and len(clss) > max_idx else -1
    # conf = float(confs[max_idx]) if confs is not None and len(confs) > max_idx else 0.0

    # # Apply YOLO confidence threshold (slider value in app)
    # threshold = det.get("yolo_conf", 0.3)  # fallback
    # if conf < threshold:
    #     return

    # # Scale to window size
    # from config import WINDOW_SCALE, CLASS_NAMES
    # sx1, sy1 = int(x1 * WINDOW_SCALE), int(y1 * WINDOW_SCALE)
    # sx2, sy2 = int(x2 * WINDOW_SCALE), int(y2 * WINDOW_SCALE)
    # w, h = sx2 - sx1, sy2 - sy1
    ...
    # # Label
    # name = CLASS_NAMES[cid] if 0 <= cid < len(CLASS_NAMES) else "Unknown"
    # label = f"{name} {conf:.2f}"
    # text_img = font.render(label, True, (255, 255, 255))
    # pad = 2
    # bg = pygame.Surface((text_img.get_width() + pad*2, text_img.get_height() + pad*2))
    # bg.fill(color)
    # surface.blit(bg, (sx1, max(0, sy1 - bg.get_height())))
    # surface.blit(text_img, (sx1 + pad, max(0, sy1 - bg.get_height()) + pad))
    ...
    while not stop_event.is_set():
        ...
        try:
            results = model.predict(
                ...
                verbose=False,
                #conf=YOLO_CONF,
                conf=state.get("yolo_conf", YOLO_CONF),
                iou=YOLO_IOU
            )
            r = results[0]
